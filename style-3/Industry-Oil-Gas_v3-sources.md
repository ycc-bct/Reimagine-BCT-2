...
6. **`#61BEE9` 偏離 DLS** —— 依指示，全頁品牌藍由 `--blue-500`（`tokens.css` 定義為 `#0092D7`）改為 `#61BEE9`。所有對比不減反增（eyebrow 5.80→9.57、按鈕 5.50→9.07）。但新值與既有的 `--blue-300`（`#5FBEEA`）逐通道僅差 (2, 0, −1)、對比 1.001:1，等於同色 —— `--blue-300` 形同冗餘，藍色階梯建議在 DLS 層級重新錨定。已在 CSS 註解標明此處偏離。
7. **本頁定位** —— nav 與 footer 是母公司 Bahwan CyberTek 的，頁內文案主體來自 BCT 子公司頁。本版的取捨是「掛在母公司網站下」，故 Solutions 與 CTA 改用 B 的文案。若最終要掛在 BCT 站下，這兩處應退回 A 的版本。
