...
export default { title: 'Foundations/Responsive' };
...
/* ---- touch targets: the rule the original page broke 28 times ---- */
export const TouchTargets = () => h(`
...
/* ---- adapt, don't shrink: the swipe rail ---- */
export const SwipeRail = () => h(`
  <div style="padding:8px">
    <div style="font:400 13.5px var(--font-body);color:var(--text-mid);max-width:64ch;margin-bottom:24px">
    ...
    <div style="display:flex;gap:14px;overflow-x:auto;scroll-snap-type:x mandatory;margin-inline:-8px;padding-inline:8px;scrollbar-width:none">
      ${['We build, run, and improve it','Platform-led, not people-led','AI in the core, not layered on','Proven at the highest stakes','Engineering accountability']
        .map(t => `
    ...
    <div style="padding:8px">
      ...
      <div style="overflow-x:auto">
        <table style="border-collapse:collapse;min-width:560px;width:100%">
          <thead><tr>
            ${['', '<640', '640', '768', '1024', '1280+'].map(x =>
              `<th style="text-align:left;padding:12px 14px;border-bottom:1px solid var(--border);font:600 11px var(--font-body);letter-spacing:.14em;text-transform:uppercase;color:var(--text-low)">${x}</th>`).join('')}
          </tr></thead>
