  ...
  <div style="padding:8px;display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px">
    ${[["Real-Time Monitoring","Integrate multi-OEM SCADA data for centralized, real-time asset monitoring."],["Digital Twin","Mirror renewable assets in real time, driving innovation through digital twin technology."],["Anomaly Detection","Continuously monitor asset health and anticipate failures with predictive alerts."]].map(([t,p])=>`
      <article style="background:var(--bg-card);border:1px solid var(--border);padding:26px 24px;transition:border-color .3s,box-shadow .3s,background .3s"
        ...
        <p style="font-size:var(--fs-body);line-height:var(--lh-body);color:var(--text-mid);max-width:34ch;margin-top:10px">${p}</p>
      </article>`).join("")}
  </div>
`),l=()=>r(`
  <div style="padding:8px">
...
`),c=()=>{const t="rvp-"+Math.random().toString(36).slice(2,8),p=r(`
    <div id="${t}" style="padding:8px">
      ...
      <div class="row" style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:24px">
        ${[0,1,2].map(e=>`
        <div class="rv" style="opacity:0;transform:translateY(28px);transition:opacity .7s cubic-bezier(.2,.6,.2,1),transform .7s cubic-bezier(.2,.6,.2,1);transition-delay:${e*.12}s;background:var(--bg-card);border:1px solid var(--border);padding:26px">
          <div style="font-family:var(--font-display);font-weight:700;font-size:var(--fs-h3);color:var(--text-hi)">0${e+1}</div>
...
\`)`,...(z=(S=i.parameters)==null?void 0:S.docs)==null?void 0:z.source}}};var M,C,D;l.parameters={...l.parameters,docs:{...(M=l.parameters)==null?void 0:M.docs,source:{originalSource:`() => h(\`
  <div style="padding:8px">
    <div style="width:44px;height:4px;background:var(--blue-500)"></div>
    <div style="font-size:var(--fs-lede);line-height:var(--lh-lede);color:var(--text-mid);margin-top:26px;max-width:46ch">
      Always 44×4px, always --blue-500. Sits above a figure or below an eyebrow.
...
\`)`,...(D=(C=l.parameters)==null?void 0:C.docs)==null?void 0:D.source}}};var $,E,T;d.parameters={...d.parameters,docs:{...($=d.parameters)==null?void 0:$.docs,source:{originalSource:`() => h(\`
  <div style="padding:8px">
    <div style="font:400 13.5px var(--font-body);color:var(--text-mid);max-width:64ch;margin-bottom:24px">
      Hairline strokes on a large viewBox. #FFFFFF primary, #A5A5A5 secondary, and exactly
      <strong style="color:var(--text-hi)">one</strong> #1294D7 filled accent per icon. Hover to see
