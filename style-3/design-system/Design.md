...
5. **`--blue-300` is a hover colour only. It is never a resting text colour.** `#5FBEEA` exists to
   mark the moment a pointer is over something — a link, an arrow, a button fill. The instant it
   is used for text that just sits there, the interface is telling the reader that a static
   paragraph is interactive, and the real hover states lose the only signal they had. Resting body
   copy is `--text-mid`; a resting accent is `--blue-500`. This holds for backgrounds too: a
   `--blue-300` fill belongs to `:hover`, not to the default state.

   ```css
   /* wrong — a paragraph that is not interactive, painted in the hover colour */
   .hero-sub { color: var(--blue-300); }

   /* right */
   .hero-sub        { color: var(--text-mid); }
   .link-arrow:hover{ color: var(--blue-300); }
   ```
...
### A last line is never a single word

No block of text may break so that its final line carries **one word on its own** — with or
without the closing full stop. It reads as a mistake, it leaves a ragged hole under the block, and
on a title it makes the second line look like a caption that got separated from something.

This is the same rule whether the block is a headline, a lede, a card paragraph or a caption, and
it applies at **every** width — a paragraph that is clean at 390px routinely strands its last word
at 414px, so it has to be checked across the range like the two-line title rule.

| Fix | When to reach for it |
|---|---|
| `text-wrap: pretty` | The default. Applied globally to `p`; the browser pulls a word down to keep the last line company. Costs nothing, so it is always on. |
| `text-wrap: balance` | Headings and short centred copy — evens every line rather than only protecting the last. Browsers stop applying it past a few lines, so it is not a fix for long paragraphs. |
| `&nbsp;` between the last two words | The deterministic fallback: it welds them so they wrap together. Use it when a specific string must hold at a specific width and `pretty` is not enough. |
| Rewrite the sentence | Last resort, and only for copy you own. Never shrink the font to make a widow go away — that trades a small blemish for an unreadable line. |

`text-wrap: pretty` is not supported everywhere, so anything that must hold — a hero headline, a
statement panel — gets the `&nbsp;` as well, not instead.
